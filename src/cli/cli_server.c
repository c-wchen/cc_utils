...
#define CMD_DEF(subcommand, help, handler) {.subcommand = subcommand, .help = help, .handler =handler}
...
int cli_create(const char *name)
{
    ...
    int ret = pthread_create(&handler->thread, NULL, msg_poll, handler);
...
int32_t bind_and_listen(const char *socket_path, int32_t *fd)
{
    ...
    if (sock_fd < 0) {
        ret = errno;
        LOG_ERROR("failed to create socket: %s", strerror(err));
        return errno;
    }
    ...
    if (bind(sock_fd, (struct sockaddr *)&address, sizeof(struct sockaddr_un)) != 0) {
        ...
        if (ret != 0) {
            LOG_ERROR("failed to bind the UNIX domain socket to %s, ret: %s", sock_path, strerror(err));
            close(sock_fd);
            return ret;
        }
    }
    ...
    if (listen(sock_fd, 5) != 0) {
        ret = errno;
        LOG_ERROR("failed to listen to socket: %s", strerror(err));
        close(sock_fd);
        return ret;
    }
    *fd = sock_fd;
...
void *msg_poll(void *arg)
{
    ...
    while (true) {
        ...
        if (ret < 0) {
            ...
            LOG_ERROR("poll(2) error: %s", strerror(err));
            return;
        }
...
comamnd_t *command_find(const char *subcommand)
{
    for (int32_t i = 0; i < 1024; i++) {
        if (commands[i].subcommand == NULL) {
            return NULL;
        }
        if (strncmp(commands[i].subcommand, subcommand, CMD_LEN) == 0) {
            return &commands[i];
        }
    }
    return NULL;
...
void do_accept(int32_t socket_fd)
{
    ...
    int rval = command_execute(msg.subcommand, argc, argv);
